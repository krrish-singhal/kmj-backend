...
const generateToken = (userId, memberId, role) => {
  return jwt.sign({ id: userId, memberId, role }, process.env.JWT_SECRET, {
...
export const register = asyncHandler(async (req, res, next) => {
  ...
  if (existingUser) {
    return next(
      new AppError("Aadhaar number already registered. Please login.", 400),
    );
  }
  ...
  if (existingDoc.exists) {
    return next(
      new AppError(
        `Member ID ${memberId} already exists. Use a different house number.`,
        400,
      ),
    );
  }
  ...
  res.status(201).json({
    success: true,
    message:
      "Registration successful! You can now login with your Member ID and Aadhaar number.",
    data: {
      user: {
        id: userPayload._id,
...
export const login = asyncHandler(async (req, res, next) => {
  ...
  const docUser = docData
    ? { ...docData, _id: docData._id || docSnap.id }
    : null;
  ...
  if (!user.password) {
    logger.warn(
      `Login blocked: user has no password (memberId=${memberIdInput})`,
    );
    return next(
      new AppError("Account not properly set up. Please contact admin.", 401),
    );
  }
  ...
  try {
    ...
    logger.warn(
      `Login bcrypt compare failed: ${error?.message || "unknown error"}`,
    );
  }
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError(
        "No user found with that Member ID and Aadhaar combination",
        404,
      ),
    );
  }
